...
impl Board {
        ...
        match self.en_passant {
            ...
            Some(x) => {
                if self.pieces(Piece::Pawn)
                    ...
                    & BitBoard::from_square(x.forward(self.side_to_move).unwrap())
                    == EMPTY
                {
                    return false;
                }
    ...
    fn set_ep(&mut self, sq: Square) {
        ...
        if get_adjacent_files(sq.get_file())
            & get_rank(sq.forward(self.side_to_move).unwrap().get_rank())
    ...
    pub fn make_move(&self, m: ChessMove, result: &mut Board) {
        *result = *self;
        ...
        if moved == Piece::Knight {
            ...
            {
                result.set_ep(dest.backward(self.side_to_move).unwrap());
